...
class CurlRajaOngkir extends CI_Controller{
...
function province(){
	...
	echo json_encode($data); 
		...
		function getCity(){
					...
					CURLOPT_HTTPHEADER => array(
							"key=> cac4f098d813679908ce542978824a7d"
					),
					));
	...
	function city($province){
			...
			echo json_encode($this->getCityOfProvince($data,$province));
				
	}

	function getCityOfProvince($array,$province){
		$result = [];
		foreach ($array as $a) {
			if( $a["province_id"] == $province ){
				$temp = [
					"city_id"	=> $a["city_id"], 
					"type" 		=> $a["type"], 
					"nama_kota" => $a["city_name"],
					"province"	=> $a["province"]
				];
